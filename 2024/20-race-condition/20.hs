...
process input = length results
  where (grid, start, end) = parse input
        results = cheatingBfs grid end 99 baselineTraversal Set.empty [] (Seq.singleton (start, 0, Virgin))
...
data State = Virgin | CheatedOnce Coord | CheatedTwice Coord Coord deriving (Show, Eq, Ord)
type Coord = (Int, Int)
type PathLength = Int
...
cheatingBfs grid end magnitude baseline visited acc ((pos, length, state):<|rest)
  ...
  where continue = cheatingBfs grid end magnitude baseline
        next =
          ...
          | neighbor <- neighbors pos
          , inRange (bounds grid) neighbor
          , Just state' <- [nextState state (grid ! neighbor) neighbor]
...
isWorseThanBaseline _ _ _ _ (CheatedOnce _) = False
isWorseThanBaseline baseline magnitude pos length (CheatedTwice _ _) = length + magnitude >= baseline Map.! pos

neighbors (y, x) = [(y+1, x), (y, x+1), (y-1, x), (y, x-1)]

nextState    Virgin            Blank _   = Just Virgin
nextState    Virgin            Wall  pos = Just $ CheatedOnce pos
