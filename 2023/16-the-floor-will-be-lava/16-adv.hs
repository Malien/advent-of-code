...
process = maximum . map energizedCount . amaFirinMahLazor . parseGrid

energizedCount = Set.size . Set.map (\(Ray pos _) -> pos)
...
parseGrid inp = listArray ((1,1), (height, width)) $ map parseTile $ filter (/= '\n') inp
...
startingRays (height, width) =
  [Ray (1     , x    ) S | x <- [1..width ]] ++
  [Ray (height, x    ) N | x <- [1..width ]] ++
  [Ray (y     , 1    ) E | y <- [1..height]] ++
  [Ray (y     , width) W | y <- [1..height]]

amaFirinMahLazor grid = map (fireRay grid Set.empty) (startingRays size)
  where ((1, 1), size) = bounds grid
...
inBounds grid (Ray (y, x) _) = x >= 1 && y >= 1 && x <= width && y <= height
  where ((1,1), (height, width)) = bounds grid
